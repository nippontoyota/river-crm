...
class UploadDuplicateTests(TestCase):
    ...
    def test_parser_marks_existing_phones_with_one_bulk_lookup(self):
        ...
        self.assertEqual(summary["removed_duplicates"], 0)
    ...
    def test_parser_requires_review_of_duplicate_phones_inside_upload(self):
        ...
        self.assertEqual(batch.skipped, 0)
        self.assertEqual([row.resolution for row in rows], [UploadRow.Resolution.PENDING, UploadRow.Resolution.PENDING, UploadRow.Resolution.IMPORT])
        self.assertEqual(rows[1].data["_duplicate_type"], "FILE")
        self.assertEqual(rows[1].data["_file_rows"], [2, 3])
        summary = UploadBatchSerializer(batch).data
        self.assertEqual(summary["removed_duplicates"], 0)
        self.assertEqual(summary["crm_duplicates_found"], 0)
        self.assertEqual(summary["file_duplicates_found"], 2)
