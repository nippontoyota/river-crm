...
def impact(user):
    ...
    return {"version": version, "assignment_role": user.role, "actionable_count": 0, "closed_count": 0,
...
def offboard(user_id, actor, action, version, reason):
    ...
    summary = {"uploads_retained": user.uploadbatch_set.count()} if user.role == User.Role.META_UPLOADER else {"branch_queue_retained": user.location}
