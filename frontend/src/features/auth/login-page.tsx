...
export function LoginPage() {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try { const result = await login(submittedEmail, submittedPassword); cacheCurrentUser(result.user); router.push(result.user.role === "META_UPLOADER" ? "/bulk-upload" : result.user.role === "SERVICE" ? "/services" : result.user.role === "FEEDBACK" ? "/feedback" : result.user.role === "CEO" ? "/ceo" : result.user.role === "ADMIN" ? "/leads" : result.user.role === "SALES_MANAGER" ? "/manager/analytics" : result.user.role === "RECEPTIONIST" ? "/capture" : result.user.role === "COMPLAINTS" ? "/complaints" : "/my-leads"); }
