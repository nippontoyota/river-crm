...
export function IntakePage() {
  ...
  const heartbeat = (name: string) => {
    ...
    return value ? `${checkedAt - new Date(value).getTime() < (name === 'processor' ? 600_000 : 120_000) ? 'Healthy' : 'Stale'} · ${displayTime(value)}` : 'No heartbeat received';
  };
  return <div className="page intake-page">
    ...
    {tab === 'connections' && <>
      <section className="panel intake-panel"><h2>Delivery health</h2>{health?.execution_mode === 'database' ? <><p>Scheduled processor: {heartbeat('processor')}</p><p className="subtext">New enquiries are processed about every five minutes. Use Fetch Meta leads to recover missed deliveries. A completed fetch means the form scan finished; imported enquiries appear in Receipts.</p></> : <><p>Worker: {heartbeat('worker')}</p><p>Scheduler: {heartbeat('scheduler')}</p><p className="subtext">Meta sends new enquiries automatically. The background worker imports them into this CRM; missed deliveries are checked every 15 minutes. Valid new leads appear in the assignment pool. Duplicate or incomplete enquiries stay in Receipts for review.</p></>}
      {health?.connections.map(c => <article className="intake-connection" key={c.id}><div className="intake-heading"><div><h3>{c.name} · {c.origin}</h3><p>{c.enabled ? 'Enabled' : 'Disabled'} · Source: {c.source} · Credentials {c.credentials_ready ? 'configured' : 'missing'}</p></div><button className="button" disabled={busy} onClick={() => void run(async () => { await api(`/api/intake/connections/${c.id}/`, { method: 'PATCH', body: JSON.stringify({ enabled: !c.enabled }) }); })}>{c.enabled ? 'Disable connection' : 'Enable connection'}</button></div>
        ...
        <label>CRM source<select value={c.source} disabled={busy} onChange={e => void run(async () => { await api(`/api/intake/connections/${c.id}/`, { method: 'PATCH', body: JSON.stringify({ source: e.target.value }) }); })}>{sources.map(s => <option key={s}>{s}</option>)}</select></label>
        {c.forms.map(f => <div className="intake-form-line" key={f.id}><div><b>{f.name}</b><small>{f.page_id ? `Page ${f.page_id} · ` : ''}Form {f.external_id}</small><small>Active from {displayTime(f.activated_at)} · {health?.execution_mode === 'database' ? 'Last successful fetch' : 'Last recovery check'} {displayTime(f.last_reconciled_at)}</small>{c.origin === 'META' && health?.execution_mode === 'database' && <small role="status">Fetch: {f.fetch_status}{f.fetch_requested_at ? ` · Requested ${displayTime(f.fetch_requested_at)}` : ''}</small>}{f.reconcile_error && <small className="intake-error">{health?.execution_mode === 'database' ? 'Fetch failed' : 'Recovery check failed'}: {f.reconcile_error}. {health?.execution_mode === 'database' ? 'Correct the issue and fetch again to resume.' : 'Correct the issue; the scheduler will retry automatically. Resume the connection if it is paused.'}</small>}</div><div className="intake-actions">{c.origin === 'META' && health?.execution_mode === 'database' && <button className="button primary" disabled={busy || !health.enabled || !c.enabled || !f.enabled || !c.credentials_ready || !!c.paused_reason || ['queued', 'running'].includes(f.fetch_status)} onClick={() => void run(async () => { await fetchMetaLeads(f.id); setNotice(`Fetch queued for ${f.name}. It will start on the next five-minute run; larger imports may take several runs.`); })}>{f.fetch_status === 'queued' ? 'Fetch queued' : f.fetch_status === 'running' ? 'Fetching…' : 'Fetch Meta leads'}</button>}<button className="button" disabled={busy} onClick={() => void run(async () => { await api(`/api/intake/forms/${f.id}/`, { method: 'PATCH', body: JSON.stringify({ enabled: !f.enabled }) }); })}>{f.enabled ? 'Disable form' : 'Enable form'}</button></div></div>)}
      </article>)}</section>
    ...
    {tab === 'mappings' && <section className="panel intake-panel"><h2>Field mappings</h2><div className="intake-grid"><label>Mapping scope<select value={mappingForm} onChange={e => { setMappingForm(e.target.value); setMappingVersion(''); }}><option value="">Choose a form</option>{forms.map(f => <option key={f.id} value={f.id}>{f.name}</option>)}</select></label><label>Version history<select value={activeMapping?.id || ''} onChange={e => setMappingVersion(e.target.value)}><option value="">New mapping</option>{scopedMappings.map(m => <option key={m.id} value={m.id}>{m.template_name || 'Form mapping'} · v{m.version} · {displayTime(m.created_at)}</option>)}</select></label></div>
      ...
      {activeMapping?.form && <button className="button" disabled={busy || !selected.length} onClick={() => void run(async () => { const result = await reprocessMapping(activeMapping.id, selected); setNotice(`${result.queued} pending receipts queued with version ${activeMapping.version}. Resolved, expired, processing or other-form receipts are skipped.`); })}>Reprocess {selected.length} selected pending receipts</button>}
    </section>}
    {detail && <div className="intake-overlay" onClick={() => setDetail(null)}><section className="panel intake-dialog" role="dialog" aria-modal="true" aria-labelledby="receipt-title" onClick={e => e.stopPropagation()}><div className="intake-heading"><div><h2 id="receipt-title">{detail.mapped_values.name || 'Review enquiry'}</h2><small>{detail.id}</small><p>{detail.state.replaceAll('_', ' ')} · {detail.form_name}</p></div><button className="button" onClick={() => setDetail(null)}>Close</button></div>
      ...
      {!terminal.includes(detail.state) && <><div className="intake-actions"><button className="button primary" disabled={busy || detail.state === 'PROCESSING'} onClick={() => resolve('correct')}>Save corrections and reprocess</button><button className="button" disabled={busy || detail.answers_expired} onClick={() => resolve('retry')}>Retry</button><button className="button" disabled={busy} onClick={() => resolve('dismiss')}>Dismiss enquiry</button></div>
        ...
        <button className="button" onClick={() => { setSample(detail.answers); setMappingForm(String(detail.form)); setMappingVersion(detail.mapping_version ? String(detail.mapping_version) : ''); setDetail(null); setTab('mappings'); }}>Map this form’s answers</button>
      </>}
      ...
      <h3>Review history</h3>{detail.history.map(event => <p key={event.id}>{event.action.replaceAll('_', ' ')} · {event.actor_name} · {displayTime(event.created_at)}</p>)}
    </section></div>}
  </div>;
}
