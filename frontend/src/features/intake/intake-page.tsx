...
import { customerFields, getIntakeHealth, getMappings, getReceipt, getReceipts, intakeStates, reprocessMapping, resolveReceipt, type CustomerValues, type Entry, type IntakeConnection, type IntakeHealth, type IntakeReceipt, type Mapping, type ReceiptDetail } from '@/lib/intake';
...
export function IntakePage() {
  ...
  const scopedMappings = mappings.filter(m => mappingForm === 'excel' ? m.form === null : m.form === Number(mappingForm));
  ...
  const heartbeat = (name: string) => {
    ...
    return value ? `${checkedAt - new Date(value).getTime() < 120_000 ? 'Running' : 'Stale'} · ${displayTime(value)}` : 'No heartbeat received';
  };
  return <div className="page intake-page">
    ...
    {tab === 'connections' && <>
      <section className="panel intake-panel"><h2>Delivery health</h2><p>Worker: {heartbeat('worker')}</p><p>Scheduler: {heartbeat('scheduler')}</p>
      {health?.connections.map(c => <article className="intake-connection" key={c.id}><div className="intake-heading"><div><h3>{c.name} · {c.origin}</h3><p>{c.enabled ? 'Enabled' : 'Disabled'} · Source: {c.source} · Credentials {c.credentials_ready ? 'configured' : 'missing'}</p></div><button className="button" disabled={busy} onClick={() => void run(async () => { await api(`/api/intake/connections/${c.id}/`, { method: 'PATCH', body: JSON.stringify({ enabled: !c.enabled }) }); })}>{c.enabled ? 'Disable connection' : 'Enable connection'}</button></div>
        ...
        {c.forms.map(f => <div className="intake-form-line" key={f.id}><div><b>{f.name}</b><small>{f.page_id ? `Page ${f.page_id} · ` : ''}Form {f.external_id}</small><small>Active from {displayTime(f.activated_at)} · Reconciled {displayTime(f.last_reconciled_at)} {f.reconcile_error}</small></div><button className="button" disabled={busy} onClick={() => void run(async () => { await api(`/api/intake/forms/${f.id}/`, { method: 'PATCH', body: JSON.stringify({ enabled: !f.enabled }) }); })}>{f.enabled ? 'Disable form' : 'Enable form'}</button></div>)}
      </article>)}</section>
    ...
    {tab === 'mappings' && <section className="panel intake-panel"><h2>Field mappings</h2><div className="intake-grid"><label>Mapping scope<select value={mappingForm} onChange={e => { setMappingForm(e.target.value); setMappingVersion(''); }}><option value="">Choose a form or template</option>{forms.map(f => <option key={f.id} value={f.id}>{f.name}</option>)}<option value="excel">Excel templates</option></select></label><label>Version history<select value={activeMapping?.id || ''} onChange={e => setMappingVersion(e.target.value)}><option value="">New mapping</option>{scopedMappings.map(m => <option key={m.id} value={m.id}>{m.template_name || 'Form mapping'} · v{m.version} · {displayTime(m.created_at)}</option>)}</select></label></div>
      ...
      {mappingForm && <MappingEditor key={`${mappingForm}:${activeMapping?.id || 'new'}`} entries={sample} form={mappingForm === 'excel' ? null : Number(mappingForm)} excel={mappingForm === 'excel'} mapping={activeMapping} onSaved={m => { setMappingVersion(String(m.id)); setNotice(`Mapping version ${m.version} saved.`); void refresh(); }} />}
