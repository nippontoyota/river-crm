...
import { api, getUpload, type UploadBatch, type UploadRow, type RtoOption, getSystemConfig } from '@/lib/crm';
...
export function UploadReview({ batch, onChange }: { batch: UploadBatch; onChange: (batch: UploadBatch) => void }) {
  const [rtos, setRtos] = useState<RtoOption[]>([]);
  ...
  useEffect(() => { void getSystemConfig().then(config => setRtos(config.rto_options || [])).catch(e => setError(e.message)); }, []);
  ...
  return <div className="intake-page upload-review">
    ...
    <p className="subtext">RTO accepts codes such as kl05, office names such as Kottayam, or both. Close spelling matches are standardized; uncertain matches need correction. Older files may leave RTO blank.</p>
