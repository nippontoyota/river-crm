...
function Metric({ label, value, suffix, detail, dark }: { label: string; value: string | number; suffix?: string; detail?: string; dark?: boolean }) { return <article className={`metric ${dark ? "metric-dark" : ""}`}><p>{label}</p><strong>{value}{suffix && <small>{suffix}</small>}</strong><span>{detail || "Live ITS data"}</span><div className="metric-mark" /></article>; }
