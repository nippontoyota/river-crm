...
import { createLead, getCurrentUser, getLeadDetail, getMyDashboard, getOfficers, sourceName, toOfficer, updateMyLead, type CurrentUser, type LeadDetail, type LeadInput, type LeadQualification, type Officer, type SalesDashboard, type SalesLead, getSystemConfig } from "@/lib/crm";
...
const emptyLead = (): LeadInput => ({ name: "", phone: "", email: "", source: "WALKIN", source_label: "", campaign: "", model_interest: "", city: "", branch: "", enquiry_date: formatDate(new Date()) });
...
export function SalesWorkspace({ followUpsOnly = false, allLeadsOnly = false, initialSection }: { followUpsOnly?: boolean; allLeadsOnly?: boolean; initialSection?: Section }) {
  ...
  return <section className="page sales-workspace">
    ...
    {editingLead && leadFields && <LeadEditPanel fields={leadFields} modelOptions={modelOptions} sourceOptions={sourceOptions.map(option => option.value)} onChange={setLeadFields} onClose={() => setEditingLead(false)} onSave={() => void saveLeadFields()} saving={saving} />}
