...
type Section = "all" | "fresh" | "followups" | "missed" | "pending" | "qualified" | "test_drive_completed" | "walkin" | "won" | "lost" | "won_lost" | "active";
...
const sections: { key: Section; label: string; count: keyof SalesDashboard["summary"]; icon: string }[] = [
  ...
  { key: "test_drive_completed", label: "Test drive completed", count: "etbr_test_drive_completed", icon: "✓" },
...
const emptyLead = (): LeadInput => ({ name: "", phone: "", email: "", source: "WALKIN", source_label: "", campaign: "", model_interest: "", city: "", rto: "", branch: "", enquiry_date: formatDate(new Date()) });
...
export function SalesWorkspace({ followUpsOnly = false, allLeadsOnly = false, initialSection }: { followUpsOnly?: boolean; allLeadsOnly?: boolean; initialSection?: Section }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      setDetail(null);
      setDraft(null);
      setNotice(!isPs && draft.call_outcome === "QUALIFIED" ? `${updated.name} qualified · Assigned PS/SO: ${updated.assignedPsName || "Not assigned"}.` : "Lead updated and follow-up history saved."); await loadDashboard();
    } catch (requestError) {
  ...
  return <section className="page sales-workspace">
    ...
    {isPs ? (
      <section className="panel sales-table-panel" style={{ background: "transparent", border: "none", boxShadow: "none", padding: 0 }}>
        {psVisibleSections.length > 0 && <nav className="sales-tabs" aria-label="All lead filters">{psVisibleSections.map(item => <button key={item.key} className={section === item.key ? "active" : ""} aria-pressed={section === item.key} onClick={() => setSection(item.key)}><i>{item.icon}</i><span>{item.label}</span><b>{summary?.[item.count] ?? 0}</b></button>)}</nav>}
    ...
    {detail && draft && <div className="modal-layer" role="presentation"><section className="modal sales-detail-modal" role="dialog" aria-modal="true" aria-labelledby="sales-detail-title">
      ...
      <div className="sales-detail-scroll">
        ...
        {!isPs && detail.ownership && <LeadOwnershipPanel ownership={detail.ownership} />}
        <FeedbackRequestForm key={`feedback-${detail.id}`} lead={detail} />
