...
type ApiLead = {
  id: number; name: string; phone: string; email: string; source: string; source_label: string; test_drive_completed_at: string | null; campaign: string; activity: string; sub_activity: string; model_interest: string; city: string; pincode: string; rto: string; generated_by: number | null;
...
type ApiSalesLead = SalesOwnershipSummary & { id: number; status: string; name: string; phone: string; source: string; flagged_to_manager: boolean };
type ApiOfficer = { id: number; first_name: string; last_name: string; email: string; phone: string; location: string; is_active: boolean };

export type Lead = {
  id: number; name: string; phone: string; source: string; sourceCode: string; model: string; city: string; pincode: string; rto: string; generated_by: number | null; enquiryDate: string | null; enquiredAt: string;
...
export type OwnerContact = { id: number; name: string; phone: string; branch: string; lifecycle_status: AccountStatus };
export type LeadOwnership = { assigned_ps: OwnerContact | null; needs_so_reassignment: boolean; branch: string; manager_branch: string; managers: OwnerContact[] };
export type SalesOwnershipSummary = { assigned_ps: number | null; assigned_ps_name: string | null; assigned_ps_status: AccountStatus | null; assigned_ps_branch: string; branch: string; needs_so_reassignment: boolean };
export type CustomerLookupResult = { id: number; name: string; phone: string; enquiry_date: string | null; status: string; branch: string; assigned_ce: Pick<OwnerContact, "id" | "name" | "lifecycle_status"> | null; needs_cre_reassignment: boolean; ownership: LeadOwnership; can_open: boolean };
export type CustomerLookupPage = { count: number; next: string | null; previous: string | null; results: CustomerLookupResult[] };
export type SalesLead = SalesOwnershipSummary & { id: number; status: string; statusCode: string; name: string; phone: string; source: string; sourceCode: string; flagged_to_manager: boolean };
export type LeadInput = { rto?: string; name: string; phone: string; email?: string; source: string; source_label?: string; campaign?: string; activity?: string; sub_activity?: string; model_interest?: string; city?: string; pincode?: string; branch?: string; enquiry_date?: string; profession?: string; ps_officer_id?: number; status?: string; category?: string; qualification?: LeadQualification; qualification_input?: LeadQualification };
...
const toSalesLead = (lead: ApiSalesLead): SalesLead => ({ ...lead, id: lead.id, name: lead.name, phone: lead.phone, source: sourceName(lead.source), sourceCode: lead.source, statusCode: lead.status, status: statusName(lead.status), flagged_to_manager: lead.flagged_to_manager });
export const toLeadDetail = (lead: ApiLead & { ownership: LeadOwnership; call_history: CallHistory[]; follow_up_history: FollowUpHistory[]; audit_history: LeadDetail["auditHistory"]; outcome_policy: OutcomePolicy; whatsapp: WhatsAppSummary }): LeadDetail => ({ ...toLead(lead), email: lead.email, sourceLabel: lead.source_label, campaign: lead.campaign, qualification: lead.qualification, callHistory: lead.call_history, followUpHistory: lead.follow_up_history, auditHistory: lead.audit_history, outcomePolicy: lead.outcome_policy, whatsapp: lead.whatsapp, ownership: lead.ownership });
...
export async function getLeadDetail(id: number) { const data = await api<ApiLead & { ownership: LeadOwnership; call_history: CallHistory[]; follow_up_history: FollowUpHistory[]; audit_history: LeadDetail["auditHistory"]; outcome_policy: OutcomePolicy; whatsapp: WhatsAppSummary }>(`/api/leads/${id}/`, { cache: "no-store" }); return toLeadDetail(data); }
export function lookupCustomer(value: string, page = 1, searchBy = "phone") {
  const query = new URLSearchParams({ query: value, search_by: searchBy, page: String(page) });
  return api<CustomerLookupPage>(`/api/leads/customer-lookup/?${query}`, { cache: "no-store" });
}
export async function completeTestDrive(id: number) {
  const data = await api<ApiLead & { ownership: LeadOwnership; call_history: CallHistory[]; follow_up_history: FollowUpHistory[]; audit_history: LeadDetail["auditHistory"]; outcome_policy: OutcomePolicy; whatsapp: WhatsAppSummary }>(`/api/leads/${id}/complete-test-drive/`, { method: "POST", body: "{}" });
  return toLeadDetail(data);
}
export async function updateWhatsAppAgreement(id: number, agreed: boolean) {
  const data = await api<ApiLead & { ownership: LeadOwnership; call_history: CallHistory[]; follow_up_history: FollowUpHistory[]; audit_history: LeadDetail["auditHistory"]; outcome_policy: OutcomePolicy; whatsapp: WhatsAppSummary }>(`/api/leads/${id}/whatsapp-agreement/`, { method: "PATCH", body: JSON.stringify({ agreed }) });
  return toLeadDetail(data);
}
...
export async function updateMyLead(id: number, payload: { whatsapp_agreed?: boolean; name?: string; phone?: string; email?: string; source?: string; source_label?: string; campaign?: string; activity?: string; sub_activity?: string; model_interest?: string; city?: string; pincode?: string; branch?: string; enquiry_date?: string | null; status?: string; category?: string; sales_outcome?: string; remarks?: string; call_status?: string; call_outcome?: string; follow_up_at?: string | null; ps_officer_id?: number; qualification?: LeadQualification; flagged_to_manager?: boolean }) { const data = await api<ApiLead & { ownership: LeadOwnership; call_history: CallHistory[]; follow_up_history: FollowUpHistory[]; audit_history: LeadDetail["auditHistory"]; outcome_policy: OutcomePolicy; whatsapp: WhatsAppSummary }>(`/api/leads/${id}/so-update/`, { method: "PATCH", body: JSON.stringify(withApiDate(payload)) }); return toLeadDetail(data); }
...
export function exportSalesManagerAnalytics(section: string, params: Record<string, string>) { const query = new URLSearchParams({ ...managerParams(params), section }).toString(); return download(`/api/analytics/sales-manager/export/?${query}`, `incheon-sales-manager-${section === "cre" ? "ce" : section}.csv`); }
...
export type UploadRowPage = { count: number; next: string | null; previous: string | null; results: UploadRow[] };
export const getUploadRows = (id: number, filter: 'invalid' | 'duplicates', page = 1) => api<UploadRowPage>(`/api/uploads/${id}/rows/?filter=${filter}&page=${page}`);
export const rejectPendingUploadRows = (id: number) => api<{ rejected: number }>(`/api/uploads/${id}/reject-pending/`, { method: 'POST', body: '{}' });
...
export type UploadRow = { file_row_count: number; file_rows: number[]; validation_errors: Record<string, string>; id: number; row_number: number; data: import("./intake").CustomerValues & { source?: string }; normalized_phone: string; validation_error: string; duplicate_of: number | null; existing_name: string; existing_status: string; duplicate_type: "CRM" | "FILE" | "INTAKE" | ""; resolution: "PENDING" | "SKIP" | "OVERWRITE" | "IMPORT" };
export type UploadBatch = { processing_mode?: 'celery' | 'database'; filename: string; mapping_version: number | null; original_deleted_at: string | null; validation_errors_found: number; id: number; status: "PARSING" | "READY" | "COMMITTED" | "FAILED"; total_rows: number; parsed_ok: number; duplicates_found: number; crm_duplicates_found: number; file_duplicates_found: number; intake_duplicates_found: number; removed_duplicates: number; pending_duplicates: number; skipped: number; error_message: string; rows?: UploadRow[] };
