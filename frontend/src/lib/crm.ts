...
const toLeadDetail = (lead: ApiLead & { ownership: LeadOwnership; call_history: CallHistory[]; follow_up_history: FollowUpHistory[]; audit_history: LeadDetail["auditHistory"]; outcome_policy: OutcomePolicy; whatsapp: WhatsAppSummary }): LeadDetail => ({ ...toLead(lead), email: lead.email, sourceLabel: lead.source_label, campaign: lead.campaign, qualification: lead.qualification, callHistory: lead.call_history, followUpHistory: lead.follow_up_history, auditHistory: lead.audit_history, outcomePolicy: lead.outcome_policy, whatsapp: lead.whatsapp, ownership: lead.ownership });
...
export function lookupCustomer(phone: string, page = 1) {
  const query = new URLSearchParams({ phone, page: String(page) });
