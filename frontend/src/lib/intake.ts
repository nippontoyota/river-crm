...
export type IntakeForm = { id: number; connection: number; name: string; external_id: string; page_id: string; enabled: boolean; activated_at: string; checkpoint: string | null; last_reconciled_at: string | null; reconcile_error: string; fetch_requested_at: string | null; fetch_status: 'idle' | 'queued' | 'running' | 'completed' | 'error'; scan_delayed: boolean };
...
export type IntakeHealth = { processor_interval_seconds: number; meta_scan_interval_seconds: number; processor_stale_after_seconds: number; meta_scan_stale_after_seconds: number; last_processor_attempt_at: string | null; last_processor_success_at: string | null; processor_delayed: boolean; enabled: boolean; execution_mode: 'celery' | 'database'; heartbeats: Record<string, string>; connections: (IntakeConnection & { counts: Partial<Record<IntakeState, number>>; oldest_pending_at: string | null; backlog_age_seconds: number; forms: IntakeForm[] })[] };
